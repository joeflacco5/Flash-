...
const mapStateToProps = ( { cards, showBack, }, { params: { deckId } }) => ({
  ...
  (!card.lastStudiedOn || (new Date - card.lastStudiedOn) / MS_IN_DAY >= card.score ))[0] // shows first card in the array!
});
  ...
  if (card) {
    ...
    <div className={ showBack ? 'back' : 'back hide' }>
      ...
      <p>
        ...
        <button onClick={e => onStudied(card.id, card.score + 1)}> Great </button>
      </p>
    </div>
    </div>)
  }
